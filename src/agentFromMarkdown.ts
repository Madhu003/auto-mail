...
async function runMarkdownAgent(): Promise<void> {
  ...
  const posts = await extractPostsFromMarkdown();
  console.log(`\n📋 ${posts.length} candidate post(s) with a contact email\n`);
...
runMarkdownAgent().catch(async (error: Error) => {
